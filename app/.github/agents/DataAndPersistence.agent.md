...
SDK: Boto3 (または Aiobotocore for Async)
...
└── converters/               # DynamoDB Item <-> Domain Model 変換
    └── dynamo_to_domain.py
コード例 (Repository Implementation):

Python

# src/infrastructure/persistence/dynamodb/repositories/user_repository.py
class DynamoUserRepository: # implements domain.UserRepository
    def __init__(self, table: Any):
        self._table = table

    async def save(self, user: User) -> Result[User, InfraError]:
        # Domain Model -> DynamoDB Item (Internal)
        item = {
            "PK": f"USER#{user.id}",
            "SK": "PROFILE",
            "name": user.name,
        ...
        try:
            ...
            return user # 保存成功時はDomain Modelをそのまま返す
        except ClientError as e:
            # エラー変換
            return InfraError(f"DynamoDB Error: {e}")
